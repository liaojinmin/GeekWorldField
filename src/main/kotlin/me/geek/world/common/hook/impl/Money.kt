...
class Money {
    ...
    private fun hook() {
        if (Bukkit.getPluginManager().getPlugin("Vault") != null) {
            Bukkit.getServer().servicesManager.getRegistration(Economy::class.java)?.let {
                GeekWorldField.say("&7软依赖 &fVault &7已兼容.")
    ...
    fun giveMoney(player: Player, amt: Double) {
        economy?.depositPlayer(player, amt) ?: GeekWorldField.say("&c经济 &fVault &c未实现，请检查经济插件！")
    }
    ...
    fun takeMoney(player: Player, amt: Double) {
        economy?.withdrawPlayer(player, amt) ?: GeekWorldField.say("&c经济 &fVault &c未实现，请检查经济插件！")
    }

    fun hasMoney(player: Player, amt: Double): Boolean {
        return economy?.has(player, amt) ?: false.also { GeekWorldField.say("&c经济 &fVault &c未实现，请检查经济插件！") }
    }
