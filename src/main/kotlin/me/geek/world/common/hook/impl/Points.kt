...
class Points {
    ...
    private fun hook() {
        ...
        if (plugin != null) {
            GeekWorldField.say("&7软依赖 &fPlayerPoints &7已兼容.")
            points = (plugin as PlayerPoints).api
        }
    }

    fun givePoints(player: UUID, amt: Int) {
        points?.give(player, amt) ?: GeekWorldField.say("&c点券 &fPlayerPoints &c未安装，请检查点券插件！")
    }

    fun takePoints(player: UUID, amt: Int) {
        points?.take(player, amt) ?: GeekWorldField.say("&c点券 &fPlayerPoints &c未安装，请检查点券插件！")
    }

    fun hasPoints(player: UUID, amt: Int): Boolean {
        val a = points?.look(player) ?: 0.also {
            GeekWorldField.say("&c点券 &fPlayerPoints &c未安装，请检查点券插件！")
            return false
        }
        return a >= amt
    }
